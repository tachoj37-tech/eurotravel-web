...
function ultimoTicket(patron) {
  let idx = -1;
  mandados.forEach(function (m, i) { if (mismo(m.to, DUENO) && (!patron || patron.test((m.text && m.text.body) || (m.interactive && m.interactive.body && m.interactive.body.text) || ''))) idx = i; });
