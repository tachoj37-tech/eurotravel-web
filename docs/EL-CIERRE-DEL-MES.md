...
## Y un dato que cambia la cadencia

**El almacén tira las conversaciones a los 45 días** (`VIDA_DIAS` en
`api/_almacen.js`).

O sea que **«cada mes» no es una costumbre cómoda: es el mínimo.** Si se salta
un mes, ese mes de conversaciones ya no existe en ningún lado — ni aquí, ni en
Supabase, ni en WhatsApp.

Los **precios no se purgan**, así que ésos no se pierden aunque pase el tiempo.

> Si quiere holgura, se sube `VIDA_DIAS`. Cuesta almacenamiento y nada más.
> Con 90 días se pueden saltar dos meses sin perder nada.

---

## El ritual, cuando el almacén ya exista

Dos comandos, el día que se cierre el mes:
...
|---|---|---|
| `cerebro/el-mes.md` | las cuentas del mes | **sí** |
| `conversaciones/AAAA-MM.md` | las conversaciones enteras | **no** |
...
nosotros. Es venta parada, y es el número que conviene ver bajar mes con mes.
...
`pruebas/probar-mes-al-cerebro.mjs` — 43 comprobaciones, sin tocar la red.

La mitad de ellas cuidan **una sola cosa, que es la irreversible**: que el
archivo que sube a Git no lleve un teléfono, ni un nombre, ni un domicilio, ni
una sola frase de nadie. Se comprueban **todos** los mensajes del mes de
prueba, uno por uno.
